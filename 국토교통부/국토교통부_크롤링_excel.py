...
async def main():
    ...
    async with async_playwright() as p:
                ...
                for row in rows:
                    ...
                    if title_td and date_td:
                        ...
                        if a_tag:
                            ...
                            # 카테고리 추출
                            category = field_td.get_text(
                                strip=True) if field_td else None
        ...
        # 상세 페이지 크롤링 병렬 처리
        semaphore = asyncio.Semaphore(5)  # 동시 작업 수 제한 (최대 5개)

        async def crawl_detail_page(entry, idx, total):
            async with semaphore:
                ...
                print(f"상세 페이지 {idx}/{total}: {href}")
                try:
                    page = await browser.new_page()
                    await page.goto(href)
                    await page.wait_for_selector('li.file')

                    # 상세 페이지의 HTML 가져오기 및 파싱
                    ...
                    if li_file:
                        ...
                        for a in a_tags:
                            ...
                            href_attr = html.unescape(
                                href_attr)  # HTML 엔티티 디코딩
                        ...
                        # 추출된 링크를 데이터에 추가
                        entry['download_link'] = download_link
                        entry['preview_link'] = preview_link
                    else:
                        entry['download_link'] = None
                        entry['preview_link'] = None
        ...
        for idx, entry in enumerate(data_list, start=1):
            tasks.append(crawl_detail_page(entry, idx, len(data_list)))
    ...
    try:
        ...
        try:
            download_col = df.columns.get_loc('download_link') + 1  # 1부터 시작
